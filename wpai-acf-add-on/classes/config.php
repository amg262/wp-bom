		...
		if ( ! is_null($section)) {
			...
			if ($filePath and ! in_array($filePath, $this->loaded)) {
                ...
                if (version_compare(phpversion(), '7.2'  , "<")){
                    $sandbox = create_function('$path', "require \$path; if(!in_array('config', array_keys(get_defined_vars()))) return array(); return \$config;");
                }
                else{
                    $sandbox = function($path) {
                        require $path;
                        if(!in_array('config', array_keys(get_defined_vars()))) return array();
                        return $config;
                    };
                }

                $config = $sandbox($filePath);
